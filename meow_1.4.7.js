...
function select_tasks() {
    ...
    if (options.indexOf(0) > -1) {
        var pat_times_list = [50, 100, 200, 400];
...
function dotask_taobao() {
    ...
    task_list.forEach(task => {
        ...
        while (textContains(task).exists) {
            ...
            else if (textContains("淘宝特价版送红包").exists()) {
            ...
            else if (textContains("续费").exists()) {
                printLog("自动跳过[88VIP续费]任务", -1);
                task_btn_index++;
            }
            else {
...
function scollSrceen(scollTimes) {
    for(var i = 0; i < scollTimes; i++) {
        ...
        var startY = parseInt(random(1500, 1750));
